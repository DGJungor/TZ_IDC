...
class PostService
{
    ...
    public function adminBaoliaoList($filter)
    {
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        if (!empty($category)) {
            ...
            array_push($join, [
                '__BAOLIAO_CATEGORY_POST__ b', 'a.id = b.post_id'
            ]);
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $join    = [
                ['__BAOLIAO_CATEGORY_POST__ relation', 'post.id = relation.post_id']
            ];
            $article = $portalPostModel->alias('post')->field('post.*')
                ->join($join)
                ->where($where)
