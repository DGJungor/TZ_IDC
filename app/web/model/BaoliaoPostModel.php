...
class BaoliaoPostModel extends Model
{
    ...
    {
        return $this->belongsToMany('BaoliaoCategoryModel', 'baoliao_category_post', 'category_id', 'post_id');
    }
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('baoliao_tag_post')->where('post_id', $articleId)->column('tag_id');
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('baoliao_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($shouldDeleteTagIds)) {
                Db::name('baoliao_tag_post')->where(['post_id' => $articleId, 'tag_id' => ['in', $shouldDeleteTagIds]])->delete();
            }

            if (!empty($data)) {
                Db::name('baoliao_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('baoliao_post')->where(['id' => $id])->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('baoliao_post')->where(['id' => ['in', $ids]])
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'web/Page/index', ['id' => $data['id']], 2, 5000);
