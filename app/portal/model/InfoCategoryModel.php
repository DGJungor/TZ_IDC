        ...
        //        }
                ...
                foreach ($categories as $item) {
                    if($item["status"]) {
                        ...
                        $item['url']     = "javascript:alert(\"此功能还未开通\")";
                        
                        $item['str_action'] = '<a href="' . url("AdminStoreCategory/add", ["parent" => $item['id']]) . '">添加子分类</a>  <a href="' . url("AdminStoreCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("AdminCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
                        array_push($newCategories, $item);
                    }
